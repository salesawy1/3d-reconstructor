...
def get_args_parser():
    ...
    parser_url.add_argument("--video_in")
...
def batch_generate(tmpdirname, model, device, image_size, server_name, server_port, file_list, silent=False):
    print("Entering batch generate...") 
    schedule = "linear"
    niter = 300
    min_conf_thr = 3
    as_pointcloud = False
    ...
    # num_files = len(file_list) if file_list is not None else 1
    num_files=6
    max_winsize = max(1, math.ceil((num_files - 1) / 2))
    winsize = max_winsize
    ...
    scene, outmodel, outgallery = get_reconstructed_scene(tmpdirname, model, device, silent, image_size, file_list, schedule, niter, min_conf_thr, as_pointcloud,
                                  mask_sky, clean_depth, transparent_cams, cam_size,
                                  scenegraph_type, winsize, refid)

    get_3D_model_from_scene(tmpdirname, silent, scene)
    return tmpdirname
...
def extract_frames(video_dir, output_dir):
    ...
    for i in range(10):
        os.makedirs(os.path.join(output_dir, f"batch_{i}"), exist_ok=True)
    ...
    for video_file in video_files:
        ...
        for batch in range(10):
            ...
            frame_pos = int(batch * 0.5 * fps)  # 0.5 seconds * fps
            ...
            if ret:
                ...
                output_path = os.path.join(output_dir, f"batch_{batch}", 
                                           f"{video_file[:-4]}_batch_{batch}.jpg")
                cv2.imwrite(output_path, frame)
            else:
                print(f"Failed to extract frame for batch {batch} from {video_file}")
